...
import { AlertTriangle, ArrowLeft, ArrowRightLeft, Check, Inbox, Monitor, RefreshCw, Split, X } from 'lucide-react';
...
function ReportDetail({ submission, data }: { submission: IntelligenceSubmission; data: InboxData }) {
  ...
  return <article className="min-w-0 px-0 py-7 lg:px-8">
    ...
    <h2 className="fi-display mt-4 text-2xl font-medium break-words sm:text-3xl">{submission.ai_summary || 'Original report'}</h2>
    ...
    {submission.processing_status === 'completed' && <section className="mt-8"><p className="fi-eyebrow">{analysisModeLabel(submission.ai_analysis_mode)}</p><div className="mt-4 grid gap-px bg-line sm:grid-cols-3"><Fact label="Category" value={submission.ai_category}/><Fact label="Product area" value={submission.ai_product_area}/><Fact label="Reproducibility" value={submission.ai_reproducibility}/><Fact label="Core workflow" value={submission.ai_core_workflow_blocked ? 'Blocked' : 'Not blocked'}/><Fact label="Keywords" value={submission.ai_keywords?.join(', ')}/><Fact label="Analysis method" value={analysisModeLabel(submission.ai_analysis_mode)}/></div>{submission.ai_severity_reasons?.length ? <ul className="mt-4 list-disc space-y-1 pl-5 text-xs text-ink-muted">{submission.ai_severity_reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul> : null}</section>}
