...
export function OwnerIssuesPage() {
  ...
  return <div className="mx-auto max-w-[1180px] px-4 py-8 sm:px-7 md:py-10">
    <div className="flex flex-col gap-5 border-b border-line pb-7 sm:flex-row sm:items-end sm:justify-between"><div className="min-w-0"><p className="fi-eyebrow">{resolvedOnly ? 'Closed loop' : 'Working queue'}</p><h1 className="fi-display mt-3 text-3xl font-medium sm:text-4xl">{resolvedOnly ? 'Resolved' : 'Issues'}</h1><p className="mt-2 text-sm text-ink-muted">{resolvedOnly ? 'Issues closed with a public explanation.' : 'Normalized problems, ordered by the work they demand.'}</p></div><div className="flex flex-wrap items-center gap-2"><Button variant="ghost"><ListFilter className="h-4 w-4" />Filter</Button><Link to="/app/setup"><Button variant="secondary"><Plus className="h-4 w-4" />New project</Button></Link></div></div>
