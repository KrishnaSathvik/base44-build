...
vi.mock('@/lib/api', () => ({ listMyIssues: vi.fn().mockResolvedValue([]) }));
...
test('shows an intentional empty issue state', async () => {
  ...
  render(<QueryClientProvider client={queryClient}><MemoryRouter initialEntries={['/app/issues']}><OwnerIssuesPage /></MemoryRouter></QueryClientProvider>);
...
test('allows long issue titles to wrap on mobile rows', async () => {
  vi.mocked(listMyIssues).mockResolvedValueOnce([{
    id: 'issue-1', public_code: 'FI-LONG01', title: 'A very long issue title that must remain readable on a narrow mobile viewport without overlapping metadata', status: 'open', severity: 'medium', report_count: 1, affected_user_count: 1,
  } as never]);
  const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } });
  render(<QueryClientProvider client={queryClient}><MemoryRouter initialEntries={['/app/issues']}><OwnerIssuesPage /></MemoryRouter></QueryClientProvider>);
