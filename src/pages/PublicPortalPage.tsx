...
import { ArrowLeft, ArrowRight, Bug, CheckCircle2, ExternalLink, Lightbulb, MessageSquareText, ShieldCheck } from 'lucide-react';
import { apiErrorMessage, getPublicProject, processFeedback, submitFeedback } from '@/lib/api';
import type { FeedbackType, SubmitFeedbackResult } from '@/lib/types';
import { BrandMark } from '@/components/Brand';
import { Button, Checkbox, Field, FileDropPlaceholder, InlineError, Input, Skeleton, Textarea } from '@/components/ui';
...
const schema=z.object({description:z.string().min(1,'Describe what happened so the team can investigate.').max(5000),expectedBehavior:z.string().max(5000).optional(),pageUrl:z.string().url('Enter a full URL, including https://').max(2000).optional().or(z.literal('')),reporterEmail:z.string().email('Enter a valid email address.').max(320).optional().or(z.literal('')),emailUpdatesEnabled:z.boolean().optional(),website:z.string().max(0).optional()});
...
export function PublicPortalPage(){
  ...
  async function onSubmit(values:FormValues){
    if(!type)return;
    setSubmitError(null);
    try{const submitted=await submitFeedback({projectSlug,submissionKey,type,description:values.description,expectedBehavior:values.expectedBehavior||undefined,pageUrl:values.pageUrl||undefined,reporterEmail:values.reporterEmail||undefined,emailUpdatesEnabled:values.emailUpdatesEnabled,website:values.website});setResult(submitted);if(!submitted.duplicate&&submitted.submissionRef){void processFeedback(submitted.submissionRef).catch(()=>undefined);}}
  ...
  return <PortalFrame productName={project.name}><div className="mx-auto max-w-2xl py-10 sm:py-16">{!type ? <>
    ...
    <h1 className="fi-display mt-4 text-4xl font-medium leading-tight sm:text-5xl">Help us make {project.name} better.</h1>
    ...
    <p className="mt-6 flex items-center gap-2 text-xs text-ink-faint"><ShieldCheck className="h-4 w-4"/>Your tracking link is private. No account is required.</p>
  </> : <>
    ...
    <p className="fi-eyebrow mt-5">{TYPES.find(option=>option.value===type)?.title}</p>
    <h1 className="fi-display mt-3 text-3xl font-medium sm:text-4xl">{type==='bug'?'What happened?':type==='feature'?'What would make this better?':'What would you like us to know?'}</h1>
    <form className="mt-8 space-y-6" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div><p className="mb-2 text-sm font-medium">Screenshot</p><FileDropPlaceholder/></div>
      <details className="rounded-lg border border-line bg-surface"><summary className="flex min-h-12 cursor-pointer list-none items-center justify-between px-4 text-sm font-medium">Add context <span className="text-xs font-normal text-ink-faint">Optional</span></summary><div className="space-y-5 border-t border-line p-4"><Field label="Page URL" htmlFor="pageUrl" error={errors.pageUrl?.message}><Input id="pageUrl" placeholder="https://…" {...register('pageUrl')}/></Field>{project.collectReporterEmail !== false && <><Field label="Email" htmlFor="reporterEmail" hint="Only used for updates" error={errors.reporterEmail?.message}><Input id="reporterEmail" type="email" autoComplete="email" {...register('reporterEmail')}/></Field><Checkbox label="Email me when the status changes" {...register('emailUpdatesEnabled')}/></>}</div></details>
      ...
      <Button type="submit" disabled={isSubmitting} className="w-full sm:w-auto">{isSubmitting?'Sending feedback…':'Send feedback'}<ArrowRight className="h-4 w-4"/></Button>
    </form>
  </>}</div></PortalFrame>;
}
