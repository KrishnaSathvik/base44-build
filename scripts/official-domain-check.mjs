...
export function validateProductionOutput(root = process.cwd()) {
  ...
  for (const required of [`rel="canonical" href="${ORIGIN}/"`,`property="og:url" content="${ORIGIN}/"`,`property="og:image" content="${ORIGIN}/og-image.png"`,`name="twitter:image" content="${ORIGIN}/og-image.png"`]) if (!index.includes(required)) errors.push(`Production metadata is missing ${required}`);
