...
try{const vercel=JSON.parse(readFileSync('vercel.json','utf8'));const rewriteOk=vercel.$schema==='https://openapi.vercel.sh/vercel.json'&&JSON.stringify(vercel.rewrites)===JSON.stringify([{source:'/(.*)',destination:'/index.html'}]);const redirectOk=JSON.stringify(vercel.redirects)===JSON.stringify([{source:'/:path*',has:[{type:'host',value:'www.vensaos.com'}],destination:'https://vensaos.com/:path*',permanent:true}]);record('Vercel SPA configuration',rewriteOk&&redirectOk?'passed':'failed',rewriteOk&&redirectOk?'':'missing SPA rewrite or www→apex redirect');}catch(error){record('Vercel SPA configuration','failed',error.message);}
