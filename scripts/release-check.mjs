...
try{const manifest=JSON.parse(readFileSync('dist/manifest.webmanifest','utf8'));const valid=manifest.name==='Feedback Inbox'&&manifest.short_name==='Feedback'&&manifest.start_url==='/'&&manifest.scope==='/'&&manifest.display==='standalone'&&manifest.theme_color==='#F6F5F1'&&manifest.background_color==='#F6F5F1';record('Manifest validation',valid?'passed':'failed');}catch(error){record('Manifest validation','failed',error.message);}
